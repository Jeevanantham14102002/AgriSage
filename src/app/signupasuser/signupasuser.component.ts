...
@Component({
  selector: 'app-signupasuser',
  templateUrl: './signupasuser.component.html',
  styleUrls: ['./signupasuser.component.css']
})
export class SignupasuserComponent {
  email: string = '';
  password: string = '';
    ...
    if (this.checkEmailAndPassword(this.email,this.password)) {
      this.SignupserviceService.signupasuser(this.email, this.password).subscribe(
        (response) => {
          ...
          this.router.navigate(['/loginasuser']);
